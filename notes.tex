...
We follow the presentation/notation of \citet{Murphy2012}.
...
$$
\mathcal L(w, V) = \sum_{i=1}^n \frac 1 2 [ f(x_i) - y_i]^2 = \sum_{i=1}^n \frac 1 2 [ w^T S(V^T x_i) - y_i]^2
$$
...
first level gradient & $\nabla_V \frac 1 2 [f(x_i)-y_i]^2 = x_i \delta_i^{vT}\in\mathbb R^{p\times u}$ &
  $\nabla_V \mathcal L(w, V) = \frac 1 n \sum_{i=1}^n x_i \delta_i^{vT} = X^T \delta^v/n$
\end{tabular}
